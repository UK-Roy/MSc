...
class SimpleGridEnv(Env):
    ...
    def create_human_patch(self):
        """
        Create a Circle patch for the human.

        @NOTE: If human position is (x,y) then, to properly render it, we have to pass (y,x) as center to the Circle patch.
        """
        return mpl.patches.Circle(
            (self.human_xy[1]+.5, self.human_xy[0]+.5), 
            0.3, 
            facecolor='blue', 
            fill=True, 
            edgecolor='black', 
    ...
    def update_human_patch(self):
        """
        @NOTE: If human position is (x,y) then, to properly 
        render it, we have to pass (y,x) as center to the Circle patch.
        """
        self.human_patch.center = (self.human_xy[1]+.5, self.human_xy[0]+.5)
        return None
